                ...
                {//targetEl, onSuccess, onError, additionalData
                    ...
                    'onSuccess' : function(response, params) {
                        ...
                        if ( response.tags.length > 0 ) {
                            $(".tagCloud").append("<div class='label label-default z-label-new'>" + response.tags + "</div> ");
                        }
                        if ( response.invalid.length > 0 ) {
                            var tagError = $(".tagError.invalid").show();
                            $(".errorMessage", tagError).text( response.invalid );
                        }
                        if ( response.inlist.length > 0 ) {
                            var tagError = $(".tagError.inlist").show();
                            $(".errorMessage", tagError).text( response.inlist );
                        }
                        $('.newTags').val('');
                    }
                }
                ...
                { //targetEl, onSuccess, onError, additionalData
                    ...
                    'onSuccess' : function(response, params) {
                        ...
                        if ( response ) {
                            ...
                            $('.tags .saveTag').first().click(oxTag.saveTag);
                            $('.tags .cancelTag').first().click(oxTag.cancelTag);
                        }
                    }
                }
            );
