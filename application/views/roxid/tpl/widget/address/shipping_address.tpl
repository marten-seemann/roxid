[{if $delivadr }]
    ...
    [{if $delivadr->oxaddress__oxstateid->value}][{$delivadr->getStateTitle()}] [{/if}]
    ...
    [{if $delivadr->oxaddress__oxcountry->value }][{ $delivadr->oxaddress__oxcountry->value }][{/if}]

    [{if ($oViewConf->getViewThemeParam('blShowFon') && $delivadr->oxaddress__oxfon->value) || ($oViewConf->getViewThemeParam('blShowFax') && $delivadr->oxaddress__oxfax->value)}]<br><br>[{/if}]
